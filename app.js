...
function renderFile(path) {
  ...
  // ✅ 핸드폰: PDF는 "버튼 눌렀을 때만" 새 탭
  if (isPdf(path) && isPhone()) {
    contentFrameWrap.innerHTML = `
      <div class="emptyState" style="line-height:1.55;">
        핸드폰에서는 PDF가 첫 페이지만 보이거나 넘김이 막히는 경우가 있어요.<br/>
        <b>아래 버튼</b>으로 열면 기기 기본 PDF 뷰어에서 편하게 넘길 수 있어!
        <div style="margin-top:14px; display:flex; gap:10px; flex-wrap:wrap;">
          <button class="primary" id="openPdfBtn" type="button">PDF 열기</button>
          <button class="ghost" id="copyLinkBtn" type="button">링크 복사</button>
        </div>
        <div style="margin-top:10px; font-size:13px; opacity:.75;">
          열고 나서 확대해서 보면 더 잘 보여.
        </div>
      </div>
    `;

    document.getElementById("openPdfBtn")?.addEventListener(
      "click",
      () => openNewTab(safe),
      { passive: true }
    );

    // (선택) 링크 복사: 모바일에서 공유/복붙 편하게
    document.getElementById("copyLinkBtn")?.addEventListener(
      "click",
      async () => {
        try {
          await navigator.clipboard.writeText(new URL(safe, window.location.href).href);
          // 알림 대신 간단히 버튼 텍스트 잠깐 바꿔도 됨
          document.getElementById("copyLinkBtn").textContent = "복사됨!";
          setTimeout(() => {
            const el = document.getElementById("copyLinkBtn");
            if (el) el.textContent = "링크 복사";
          }, 1200);
        } catch (e) {
          // clipboard가 막혀있으면 그냥 새 탭 안내
          openNewTab(safe);
        }
      },
      { passive: true }
